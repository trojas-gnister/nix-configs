...
def select_device():
    # TODO: check free space of device and if it isn't over 10GB then throw error
    # TODO: check devices and not partitions
...
def partition_device(device, swap_size):
    ...
    try:
        # TODO: efi make optional
        subprocess.run(
            ["sgdisk", "-n", "1:0:+512M", "-t", "1:EF00", f"/dev/{device}"], check=True
        )
        subprocess.run(
            ["sgdisk", "-n", f"2:0:+{swap_size}G", "-t", "2:8300", f"/dev/{device}"],
            check=True,
        )
        subprocess.run(
            ["sgdisk", "-n", "3:0:0", "-t", "3:8300", f"/dev/{device}"], check=True
        )
...
def mount_partitions(device, encrypt):
    try:
        ...
        if encrypt:
            ...
            # TODO: EFI. handle if EFI already exists and isn't created with script
